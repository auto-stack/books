...
* **`fn main()` 的家主地位**：在 C++ 或 Java 中，`main` 只是一个普通的入口函数。但在 Auto 中，`main` 是整个 Actor 宇宙的**“创世进程”**。在底层的物理意义上，`main` 运行在隔离的主线程中。它的职责是部署资源、派生（Spawn）其他的 Task，并在一切就绪后接管当前线程，将控制权彻底移交给底层的 Tokio 调度系统。
...
#### 编译为终极形态 (AOT 模式)
如果你想把这个程序部署到没有安装 Auto 环境的服务器上，只需通过 `auto build` 调用重型装备（底层会自动路由给 `autoc`）：
