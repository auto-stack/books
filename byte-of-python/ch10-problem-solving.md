...
We have explored various parts of the Auto language and now we will take a look at how all these parts fit together, by designing and writing a program which _does_ something useful. The idea is to learn how to write a Auto script on your own.
...
Since Auto transpiles to Python via `a2p`, we can use Python's standard library modules such as `os` and `time` in our Auto code. The `use` keyword in Auto becomes `import` in the generated Python.

Save as `backup_ver1.at`:

<Listing number="10-1" file-name="backup_ver1.auto" caption="The backup script - version 1">
...
The `time.strftime()` function takes a format string such as the one we have used in the above program. The `%Y` format code will be replaced by the year with the century. The `%m` format code will be replaced by the month as a decimal number between `01` and `12` and so on. The complete list of such format codes can be found in the [Python Reference Manual](http://docs.python.org/3/library/time.html#time.strftime).

We create the name of the target zip file using the `+` operator which _concatenates_ the strings i.e. it joins the two strings together and returns a new one. Then, we create a string `zip_command` using an f-string (`f"..."`) with `$variable` interpolation, which becomes `{variable}` in the Python output. The `$` syntax is Auto's way of embedding expressions in strings, while Python uses `{}`.

The `zip` command that we are using has some options available, and one of these options is `-r`. The `-r` option specifies that the zip command should work **r**ecursively for directories, i.e. it should include all the subdirectories and files. Options are followed by the name of the zip archive to create, followed by the list of files and directories to backup. We convert the `source` list into a string using the `join` method.

Then, we finally *run* the command using the `os.system` function which runs the command as if it was run from the *system* i.e. in the shell - it returns `0` if the command was successful, else it returns an error number.
...
>
> Auto uses `!` for logical negation (not) instead of Python's `not` keyword. The `a2p` transpiler converts `!expr` to `not expr` automatically. Similarly, Auto's `$var` inside f-strings becomes `{var}` in Python.
...
Save as `backup_ver2.at`:

<Listing number="10-2" file-name="backup_ver2.auto" caption="The backup script - version 2 (daily backup naming)">
...
Save as `backup_ver3.at`:

<Listing number="10-3" file-name="backup_ver3.auto" caption="The backup script - version 3 (with user comment - BUGGY!)">
...
> **Note for Auto Programmers:**
>
> This is a great example of one of Auto's design advantages over Python. In Auto, statements end with `}` (for blocks) or with the natural line boundaries within a block, so line continuation issues like this one simply don't arise. Auto's block-based syntax with `{}` makes it clear where statements begin and end. The version 4 listing will show how we can fix this more elegantly in Auto using an `if/else` expression.

## Fourth Version

Save as `backup_ver4.at`:

<Listing number="10-4" file-name="backup_ver4.auto" caption="The backup script - version 4 (fixed with user comment)">
...
Notice how we fixed the bug from version 3. In the Auto code, we use an `if/else` expression that assigns directly to `let target`, which cleanly avoids the line continuation issue entirely. The `a2p` transpiler converts this to a standard Python `if/else` statement with a backslash continuation. This is an example of how Auto's syntax can help you avoid common Python pitfalls.
...
We have seen how to create our own Auto programs/scripts and the various stages involved in writing such programs. You may find it useful to create your own program just like we did in this chapter so that you become comfortable with Auto as well as problem-solving.

Key takeaways for this chapter:

- **`use` for Python modules** -- Auto can use Python's standard library modules like `os`, `time`, and `sys` via the `use` keyword. The `a2p` transpiler converts these to `import` statements.
- **`os.system()`** -- runs a shell command and returns the exit code. A return value of `0` means success.
- **`os.sep`** -- the OS-specific path separator (`/` on Unix, `\\` on Windows).
- **`os.path.exists()`** and **`os.mkdir()`** -- check if a directory exists and create directories.
- **`time.strftime()`** -- format dates and times as strings.
- **`input()`** -- read user input from the console.
- **`.len()`** -- get the length of a string (Auto style; Python uses `len()`).
- **`.replace()`** -- replace substrings within a string.
- **`.join()`** -- join list elements into a string with a separator.
- **f-strings with `$var`** -- Auto's string interpolation syntax, transpiled to Python's f-string `{var}` syntax.
- **`if/else` expressions** -- Auto supports `if/else` as expressions that return values, helping avoid common bugs like missing line continuations.
