...
replaces the ```auto and ```rust code blocks with content from
the corresponding listing directory.

Usage:
    python scripts/sync_listings.py [--dry-run] [--chapters ch01 ch02 ...]

Source of truth: listings/chXX/listing-X-Y/main.at and main.expected.rs
"""
...
BOOK_DIR = Path(__file__).parent.parent / "rust"
...
def read_listing_code(listing_dir: Path) -> tuple[str, str] | None:
    """Read Auto and expected Rust code from listing directory."""
    at_file = listing_dir / "main.at"
    rs_file = listing_dir / "main.expected.rs"
    ...
    rust_code = ""
    if rs_file.exists():
        rust_code = rs_file.read_text(encoding="utf-8").rstrip()

    return auto_code, rust_code
...
def replace_listing_block(content: str, dry_run: bool = False) -> tuple[str, list[str]]:
    ...
    the ```auto and ```rust code blocks inside them.
    """
    changes = []
    result = content
    ...
    def replacer(match: re.Match) -> str:
        ...
        # Replace ```rust block (if exists)
        if rust_code:
            new_body = re.sub(
                r"(```rust[^\n]*\n)(.*?)(```)",
                lambda m: f"{m.group(1)}{rust_code}\n{m.group(3)}",
                new_body,
                count=1,
                flags=re.DOTALL,
            )
